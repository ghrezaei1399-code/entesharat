...
(function() {
    ...
    const clearBtn = document.getElementById('clearPosterBtn');
    ...
    function generatePoster() {
        ...
        showStatus('⏳ در حال تولید پوستر با هوش مصنوعی...', true);

        setTimeout(function() {
            // تولید یک شناسه منحصر‌به‌فرد برای پوستر
            const posterId = Date.now();
            const imageUrl = 'images/bestclasick.png'; // تصویر پیش‌فرض
    ...
    function clearForm() {
        ...
        posterEmail.value = '';
        posterDesc.value = '';
        posterStatus.className = 'poster-status-modern';
    ...
    // بازیابی پوسترهای ذخیره شده
    function loadSavedPosters() {
        const posters = JSON.parse(localStorage.getItem('generatedPosters') || '[]');
        if (posters.length > 0 && posterGallery) {
            // فقط آخرین ۵ پوستر را نمایش بده
            const recent = posters.slice(-5).reverse();
            recent.forEach(function(p) {
                const posterEl = document.createElement('div');
                posterEl.className = 'gallery-card-modern';
                posterEl.style.animation = 'fadeSlide 0.5s ease';
                posterEl.innerHTML = `
                    <img src="images/bestclasick.png" alt="${p.title}" class="card-image" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22190%22%3E%3Crect width=%22200%22 height=%22190%22 fill=%22rgba(108,92,231,0.04)%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 font-family=%22Tahoma%22 font-size=%2214%22 fill=%22rgba(45,27,78,0.2)%22%3E${p.title}%3C/text%3E%3C/svg%3E'">
                    <div class="card-overlay">📅 ${p.date}</div>
                    <div class="card-info">
                        <span class="badge-ai">🤖 تولید هوش مصنوعی</span>
                        <h4>${p.title}</h4>
                ...
                `;
                posterGallery.appendChild(posterEl);
            });
        }
    }

    // بارگذاری پوسترهای ذخیره شده
    loadSavedPosters();

    console.log('✅ ماژول ساخت پوستر: راه‌اندازی شد');
})();  
